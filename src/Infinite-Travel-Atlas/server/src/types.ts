...
export interface TravelPlanResponse {
  location: TravelPlanLocation;
  overview: string;
  best_time_to_go: string;
  ideal_for: string;
  itinerary: TravelPlanItineraryDay[];
  highlights: string[];
...
export interface TravelGeoMatch {
...
export interface TravelNearbyFilters {
  theme?: DestinationTheme;
  minEnergy?: number;
}

export interface TravelNearbyResult {
  center: { lat: number; lon: number };
  radiusKm: number;
  matches: TravelGeoMatch[];
  totalCandidates: number;
  source: string;
  cached?: boolean;
  filters?: TravelNearbyFilters;
}

...
import { buildAgentTravelContext, hasTravelAugmentation } from "../utils/agentContext";
...
export function DestinationGuideRoute() {
  ...
  const isTravelAugmentedMeta = hasTravelAugmentation(destinationMeta);
  const travelAgentContext = useMemo(
    () => (isTravelAugmentedMeta ? buildAgentTravelContext(destinationMeta) : undefined),
    [destinationMeta, isTravelAugmentedMeta]
  );
  const destinationKey = destinationMeta
    ? (isTravelAugmentedMeta && destinationMeta.travelNodeId) || destinationMeta.slug || destinationMeta.iso2
    : "unknown";

  const query = useQuery<AgentContentResponse, Error>({
    queryKey: ["agent-guide", destinationKey, theme, language],
    queryFn: () => {
      if (!destinationMeta) {
      ...
      return generateAgentContent({
        ...
        theme,
        language,
        ...(travelAgentContext ? { travelContext: travelAgentContext } : {})
      });
    },
    ...
    enabled: Boolean(destinationMeta)
  });

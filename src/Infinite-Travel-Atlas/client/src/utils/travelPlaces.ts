...
interface TravelPlaceTransformMeta {
  distanceKm?: number;
  bearing?: number;
  radiusKm?: number;
  queryCenter?: [number, number];
  source?: string;
}

export function transformTravelPlace(
  place: WorldTravelPlace,
  meta?: TravelPlaceTransformMeta
): TravelAugmentedDestination {
  ...
  return {
    ...
    ...(meta?.distanceKm !== undefined ? { geoDistanceKm: meta.distanceKm } : {}),
    ...(meta?.bearing !== undefined ? { geoBearing: meta.bearing } : {}),
    ...(meta?.radiusKm !== undefined ? { geoRadiusKm: meta.radiusKm } : {}),
    ...(meta?.queryCenter ? { geoQueryCenter: meta.queryCenter } : {}),
    ...(meta?.source ? { geoSource: meta.source } : {})
  };
}
